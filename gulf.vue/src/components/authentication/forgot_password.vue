<template>
  <div class="center">
    ...
    <md-card md-with-hover v-if="showCard">
      <transition name="fadeReg" mode="out-in" appear>
        <div @keydown.enter="callAction()">
          ...
          <md-card-content v-else>
            <div v-for="phone in userInfo.phones" :key="phone">
              Phone: {{ hideNumber(phone) }}
              <md-radio class="radio" v-model="radio" :value="phone+'-Call'">Call</md-radio>
              <md-radio class="radio" v-model="radio" :value="phone+'-SMS'">SMS</md-radio>
            </div>
...
const checkEmail = (value, component) => {
  component.emailError =
    component.checkedEmail && !component.validEmail
      ? "Email provided is not in our database!"
    ...
    contactInfo() {
      ...
      AuthBus.ContactInfo(this.email, (err, data) => {
        ...
        if (data) {
          ...
          this.radio = this.userInfo.emails[0];
        } else {
          this.modalMsg = err;
          this.showModal = true;
        }
